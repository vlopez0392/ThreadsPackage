...
;------------------------------------------------------------
;	preemptive.c:49: void Bootstrap(void) {   
;	-----------------------------------------
;	 function Bootstrap
;	-----------------------------------------
_Bootstrap:
	...
	ar1 = 0x01
	ar0 = 0x00
;	preemptive.c:51: TMOD = 0;  // timer 0 mode 0
	mov	_TMOD,#0x00
;	preemptive.c:52: IE = 0x82;  // enable timer 0 interrupt; keep consumer polling
	mov	_IE,#0x82
;	preemptive.c:54: TR0 = 1; // set bit TR0 to start running timer 0
;	assignBit
	setb	_TR0
;	preemptive.c:57: threadBitMask = (char)0x00;
	mov	_threadBitMask,#0x00
;	preemptive.c:60: currThr = ThreadCreate(main);
	mov	dptr,#_main
	lcall	_ThreadCreate
	mov	_currThr,dpl
...
;------------------------------------------------------------
;	preemptive.c:65: ThreadID ThreadCreate(FunctionPtr fp){
;	-----------------------------------------
;	 function ThreadCreate
;	-----------------------------------------
_ThreadCreate:
;	preemptive.c:66: EA =0; //Critical section
;	assignBit
	clr	_EA
;	preemptive.c:69: if(threadBitMask == (char)0x0F){
	mov	a,#0x0f
...
;	preemptive.c:79: while(tempBitMask>>=1){
	mov	r7,#0x00
00103$:
	mov	a,_tempBitMask
	clr	c
	...
	mov	_tempBitMask,r6
	jz	00105$
;	preemptive.c:80: threadId++;
	inc	r7
	sjmp	00103$
00105$:
;	preemptive.c:84: char startingSP = (char)((threadId^(0x01<<2))<<4);
	mov	a,#0x04
	xrl	a,r7
	swap	a
	anl	a,#0xf0
	mov	r6,a
;	preemptive.c:87: tempSP = SP; 
	mov	_tempSP,_SP
;	preemptive.c:88: SP = startingSP;
	mov	_SP,r6
;	preemptive.c:91: tempPSW ^= (char)(threadId<<3);
	mov	ar6,r7
	mov	a,r6
	swap	a
	rr	a
...
;------------------------------------------------------------
;	preemptive.c:116: void myTimer0Handler(void){
;	-----------------------------------------
;	 function myTimer0Handler
;	-----------------------------------------
_myTimer0Handler:
;	preemptive.c:117: EA = 0; //Critical section
;	assignBit
	clr	_EA
;	preemptive.c:119: SAVESTATE;
	PUSH ACC 
	PUSH B 
	PUSH DPL 
	PUSH DPH 
...
;	preemptive.c:123: do{
00106$:
;	preemptive.c:124: if(shift == (char)0x08){
	mov	a,#0x08
	cjne	a,_shift,00102$
;	preemptive.c:125: shift = 0x01;
	mov	_shift,#0x01
;	preemptive.c:126: pos = 0;
	mov	r7,#0x00
	sjmp	00103$
00102$:
...
;	preemptive.c:129: pos++;
	inc	r7
00103$:
;	preemptive.c:131: if(shift & threadBitMask){
	mov	a,_threadBitMask
	anl	a,_shift
	jz	00106$
;	preemptive.c:132: currThr = pos;
	mov	_currThr,r7
...
;pos                       Allocated to registers r7 
;------------------------------------------------------------
;	preemptive.c:145: void ThreadYield(void) {
;	-----------------------------------------
;	 function ThreadYield
;	-----------------------------------------
_ThreadYield:
;	preemptive.c:146: SAVESTATE;
	PUSH ACC 
	PUSH B 
	PUSH DPL 
	PUSH DPH 
...
;	preemptive.c:151: do{
00106$:
;	preemptive.c:152: if(shift == (char)0x08){
	mov	a,#0x08
	cjne	a,_shift,00102$
;	preemptive.c:153: shift = 0x01;
	mov	_shift,#0x01
;	preemptive.c:154: pos = 0;
	mov	r7,#0x00
	sjmp	00103$
00102$:
...
;	preemptive.c:157: pos++;
	inc	r7
00103$:
;	preemptive.c:159: if(shift & threadBitMask){
	mov	a,_threadBitMask
	anl	a,_shift
	jz	00106$
;	preemptive.c:160: currThr = pos;
	mov	_currThr,r7
