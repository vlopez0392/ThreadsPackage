...
Project Checkpoint 4 - preemptive.c
...
__data __at (0x3C) static char waitProducer;           //Producer to Wait 
...
//ThreadYield - Version 2 - Works much better (1,2,3) - UNFAIR (4)
...
//ISR- ThreadYield() + Thread selection restrictions - FAIR (4)
void myTimer0Handler(void){
    EA = 0; //Critical section
    SAVESTATE;
    //Restricted round-robin scheduling policy - Choose next thread available 
    /*Rules to avoid starvation: 1. Move from producer to consumer 
                                 2. Move from consumer to producer. Producer must wait
                                 if it has already produced.
    */
    shift = (char)0x01 << currThr;
    do{
        if(shift == (char)0x08){
        ...
        if(shift & threadBitMask){
            if(currThr != 0){
                ...
                currThr = waitProducer; //Producer waiting gets to run
                if(waitProducer == 1){
                    waitProducer = 2;
                }else{
                    waitProducer = 1;
