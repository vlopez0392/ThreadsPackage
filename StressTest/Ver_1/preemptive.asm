...
;	preemptive.c:59: waitProducer = (char)0x01;
	mov	_waitProducer,#0x01
...
;	preemptive.c:89: SP = startingSP;
	mov	_SP,r6
;	preemptive.c:92: tempPSW = (char)0x00;
	mov	_tempPSW,#0x00
;	preemptive.c:93: tempPSW ^= (char)(threadId<<3);
	mov	ar6,r7
	mov	a,r6
	swap	a
	rr	a
...
;	preemptive.c:104: __endasm;
	...
	PUSH	_pushZero ;; DPH
	PUSH	_tempPSW ;; PSW
;	preemptive.c:107: SPArray[threadId] = SP;
	mov	a,r7
	add	a,#_SPArray
	mov	r0,a
	mov	@r0,_SP
;	preemptive.c:110: SP = tempSP;
	mov	_SP,_tempSP
;	preemptive.c:112: EA = 1;
;	assignBit
	setb	_EA
;	preemptive.c:114: return threadId;
	mov	dpl,r7
;	preemptive.c:115: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	preemptive.c:118: void ThreadYield(void) {
;	-----------------------------------------
...
;	-----------------------------------------
_myTimer0Handler:
;	preemptive.c:158: EA = 0; //Critical section
;	assignBit
	clr	_EA
;	preemptive.c:159: SAVESTATE;
	...
	mov	r0,a
	mov	@r0,_SP
;	preemptive.c:165: shift = (char)0x01 << currThr;
	mov	b,_currThr
	inc	b
	mov	a,#0x01
	sjmp	00139$
...
;	preemptive.c:166: do{
00112$:
;	preemptive.c:167: if(shift == (char)0x08){
	mov	a,#0x08
	cjne	a,_shift,00102$
;	preemptive.c:168: shift = 0x01;
	mov	_shift,#0x01
	sjmp	00103$
00102$:
;	preemptive.c:170: shift = (shift << 1) | (shift >>7);
	mov	a,_shift
	rl	a
	mov	_shift,a
00103$:
;	preemptive.c:172: if(shift & threadBitMask){
	mov	a,_threadBitMask
	anl	a,_shift
	jz	00112$
;	preemptive.c:173: if(currThr != 0){
	mov	a,_currThr
	jz	00108$
;	preemptive.c:174: currThr = 0;
	mov	_currThr,#0x00
	sjmp	00114$
00108$:
;	preemptive.c:176: currThr = waitProducer; //Producer waiting gets to run
	mov	_currThr,_waitProducer
;	preemptive.c:177: if(waitProducer == 1){
	mov	a,#0x01
	cjne	a,_waitProducer,00105$
;	preemptive.c:178: waitProducer = 2;
	mov	_waitProducer,#0x02
	sjmp	00114$
00105$:
;	preemptive.c:180: waitProducer = 1;
	mov	_waitProducer,#0x01
;	preemptive.c:185: }while(1);
00114$:
;	preemptive.c:186: RESTORESTATE;
	mov	a,_currThr
	add	a,#_SPArray
	mov	r1,a
	mov	_SP,@r1
